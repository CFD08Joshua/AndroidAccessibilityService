...
import com.example.androidaccessibilityservice.util.DisplayUtil;
import com.example.androidaccessibilityservice.util.SystemBrightnessUtil;
import com.example.androidaccessibilityservice.util.SystemVolumeUtil;
...
class ControlBar {
    ...
    LinearLayout getView(final Context context, boolean left, int tag, final SideBarContent sideBarContent) {
        ...
        if(tag == 0) {
            ...
            seekBar.setMax(255);
            seekBar.setProgress(SystemBrightnessUtil.getBrightness(context));
            seekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
                @Override
                public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                    SystemBrightnessUtil.setBrightness(context, progress);
            ...
            // volume range 0~15
            seekBar.setMax(15);
            seekBar.setProgress(SystemVolumeUtil.getVolume(context));
            seekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
                @Override
                public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                    SystemVolumeUtil.setVolume(context, progress);
                    sideBarContent.removeOrSendMsg(true,true);
                }
